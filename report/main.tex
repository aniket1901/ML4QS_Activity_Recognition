...
\label{tab:features}
\centering\small
\begin{tabular}{lp{1.8cm}p{4.6cm}p{3.3cm}}
\toprule
...
Table~\ref{tab:results} lists every model with its best setting, grouped-CV accuracy, and held-out test accuracy and macro-F1. Figure~\ref{fig:comparison} plots the four test metrics with the random baseline.

\begin{table}[ht]
\caption{All models, sorted by test macro-F1. Grouped-CV is on training subjects; test is the held-out subject\_2.}
\label{tab:results}
\centering\small
\begin{tabular}{lp{4.6cm}ccc}
\toprule
\textbf{Model} & \textbf{Best hyperparameters} & \textbf{CV acc} & \textbf{Test acc} & \textbf{Test F1} \\
\midrule
\textbf{Stacking (SVM+RF+kNN+DT $\to$ LR)} & tuned bases; LogReg meta & 0.9920 & \textbf{0.9295} & \textbf{0.9287} \\
Voting (hard) & tuned SVM+RF+kNN+DT & 0.9924 & 0.9277 & 0.9271 \\
Voting (soft) & tuned SVM+RF+kNN+DT & 0.9929 & 0.9223 & 0.9213 \\
Gradient Boosting & lr 0.1, depth 2, 200 trees & 0.9893 & 0.9036 & 0.9016 \\
SVM (RBF) & $C{=}100$, $\gamma{=}0.001$ & 0.9915 & 0.8893 & 0.8892 \\
kNN & $k=3$ & 0.9920 & 0.8848 & 0.8840 \\
Naive Bayes & defaults & 0.9353 & 0.8812 & 0.8792 \\
Random Forest & 300 trees, depth None, mf 0.3 & 0.9879 & 0.8804 & 0.8762 \\
Decision Tree & depth 7 & 0.9362 & 0.8250 & 0.8197 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.62\textwidth]{figures/cm_best.png}
